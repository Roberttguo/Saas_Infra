...
class TemplateMixin(object):
    ...
    fn=os.path.join(os.path.dirname(__file__), "templates/report.html")
    HTML_TEMPLATE = open(fn, 'r').read().encode('utf-8')
    ...
    #fn =(os.path.join(os.path.dirname(__file__),'templates/head_inserts.html')
    fn=os.path.join(os.path.dirname(__file__), "templates/head_inserts.html")
    STYLESHEET_TEMPLATE = open(fn, 'r').read() \
        .encode('utf-8')
    ...
    fn=os.path.join(os.path.dirname(__file__), "templates/header.html")
    HEADING_TEMPLATE = open(fn, 'r').read() \
        .encode('utf-8')

    # variables: (name, value)
    fn = os.path.join(os.path.dirname(__file__), "templates/header_parameters.html")
    HEADING_ATTRIBUTE_TEMPLATE = open(fn, 'r')\
        .read().encode('utf-8')
    ...
    fn = os.path.join(os.path.dirname(__file__), "templates/result_table.html")
    REPORT__TABLE_TEMPLATE = open(fn, 'r') \
        .read().encode('utf-8')

    # variables: (style, desc, count, Pass, fail, error, cid)
    fn = os.path.join(os.path.dirname(__file__), "templates/test_class.html")
    REPORT_CLASS_TEMPLATE = open(fn, 'r').read()\
        .encode('utf-8')

    # variables: (tid, Class, style, desc, status)
    fn = os.path.join(os.path.dirname(__file__), "templates/report_test_with_output.html")
    REPORT_TEST_WITH_OUTPUT_TMPL = \
        open(fn, 'r').read()\
        .encode('utf-8')

    # variables: (tid, Class, style, desc, status)
    fn = os.path.join(os.path.dirname(__file__), "templates/report_test_no_output.html")
    ...
    # ENDING
    # ------------------------------------------------------------------- #
    fn = os.path.join(os.path.dirname(__file__), "templates/footer.html")
    ENDING_TEMPLATE = open(fn, 'r').read()\
        .encode('utf-8')
